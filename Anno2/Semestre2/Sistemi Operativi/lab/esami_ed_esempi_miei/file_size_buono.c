...
void dir_thread(void *arg) {
    ...
    printf(YEL"[D-%lu]"RES" scansione della cartella "EVI"%s"RES"\n", td->thread_n,
           td->pathdir);
    ...
    while ((entry = readdir(dp))) {
        ...
        if (S_ISREG(statbuf.st_mode)) {
            ...
            printf(YEL"[D-%lu]"RES" trovato il file "EVI"%s"RES" in "EVI"%s"RES"\n", td->thread_n,
    ...
    if(!reg){
        printf(RED"[D-%lu]"RES" la cartella "EVI"%s"RES" non contiene file regolari\n", td->thread_n, td->pathdir);
        // verifico se uno slot della coda risulta libero
            if ((err = sem_wait(&td->sh->empty)) != 0)
                exit_with_err("sem_wait", err);
            ...
            // aggiungo all'interno della struttura dati condivisa il nuovo
            // pathfile
            strncpy(td->sh->pathfiles[td->sh->index_in], VUOTO, PATH_MAX);
...
void stat_thread(void *arg) {
    ...
    while (!done) {
        ...
        printf(GRE"[STAT]"RES" il file "EVI"%s"RES" ha dimensione "MAG"%lu byte"RES"\n", filepath,
               statbuf.st_size);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (sp->done)
            ...
            printf(BLU"[MAIN]"RES" con il file "EVI"%s"RES" il totale parziale è di "MAG"%lu byte"RES"\n",
    ...
    printf(BLU"[MAIN]"RES" il totale finale è di "MAG"%lu byte"RES"\n", total_bytes);
