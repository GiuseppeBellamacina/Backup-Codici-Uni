...
private:
	...
	{ // si può chiamare moveNodeUp
		if (parent->getNodeParent() != NULL)
		{
			...
			this->setRoot(child);
		child->setNodeParent(parent->getNodeParent()); // imposto come parent di child, il suo precedente nonno
		parent->setNodeParent(child); // imposto come parent di parent, il suo precedente figlio
	}
	...
	void leftRotate(Node<T>* node) // ricorda, il nodo è il perno di rotazione
	{ // NodeLeft = LeftChid
	...
	void iterativeInsertFixUp2(Node<T>* key) // fix red red at given node
	{ // vedere se funzione sta funzione
		// key è al suo posto ed è rosso
		Node<T>* node = key;
        while(true) // trovare altra condizione
		{
			if(node == getRoot())
			{
				node->setColor(BLACK);
