...
// contiene delle costanti (serve per INT_MIN)
#include <limits>
using namespace std;

// Soluzione 1: ricerca esaustiva
int sommaMassima1(int S[], int n){ // O(n^3)
	int _max = INT_MIN; // minimo intero possibile
	int somma;
...
// Soluzione 2:
/*
	Se ho una somma(S[i,j-1]) la sfrutto
	per calcolare la somma(S[i,j]), infatti
	somma(S[i,j]) = somma(S[i,j-1]) + S[j]
*/
int sommaMassima2(int S[], int n){ // O(n^2)
	int _max = 0;
	int somma;
	// fisso l'elemento iniziale della sequenza
	for(int i=0; i<n; i++){ // O(n)
		somma = 0;
...
// Trovare somma max in arr[l...h] t.c. il segmento include arr[m]
...
int maxSubarraySum(int arr[], int l, int h){
	...
	Restituisco il massimo tra tre valori:
		...
		c) max somma considerando sequenze che includeono arr[m]
	*/
