...
//  MY_CONSTANT=10; you cannot assign again a value to a constant
   ...
   */
   ...
   /* Errori di approssimazione (troncamento)
    La radice di 2 è un numero irrazionale, la sua computazione sarà as un certo punto "troncata" quindi calcoliamo un valore troncato
   */
   double r = sqrt(2.0); //irrational number!
   cout << "\n"; 
   cout << "sqrt(2) squared is not 2 but " << setprecision(20) << r * r << endl;

   //The value 0.1 cannot be codified with a finite number of 
   //elements.
   cout << fixed << setprecision(10) <<  0.1 << endl; 
   cout << fixed << setprecision(15) <<  0.1 << endl; 
   cout << fixed << setprecision(20) <<  0.1 << endl; 
   // but it holds for the number 0.125
   cout << fixed << setprecision(20) <<  0.125 << endl; 
